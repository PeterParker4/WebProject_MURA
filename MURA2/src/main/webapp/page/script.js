...
function inputCheck() {
	...
	else if(document.signinForm.pass_mem.value=="") {
		alert("비밀번호를 입력해주세요.");
		document.signinForm.pass_mem.focus();
		return;
	}
	else if(document.signinForm.repass_mem.value=="") {
		alert("비밀번호를 확인해주세요.");
		document.signinForm.repass_mem.focus();
		return;
	}
	else if(document.signinForm.pass_mem.value!=document.signinForm.repass_mem.value) {
		alert("비밀번호가 일치하지 않습니다.");
		document.signinForm.repass_mem.focus();
		return;
	}
	else if(document.signinForm.name_mem.value=="") {
		alert("이름을 입력해주세요.");
		document.signinForm.name_mem.focus();
		return;
	}
	else if(document.signinForm.tel_mem.value=="") {
		alert("전화번호를 입력해주세요.");
		document.signinForm.tel_mem.focus();
		return;
	}
	else if(document.signinForm.email_mem.value=="") {
		alert("이메일을 입력해주세요.");
		document.signinForm.tel_mem.focus();
		return;
	}
	else if(document.signinForm.nn_mem.value=="") {
		alert("닉네임을 입력해주세요.");
		document.signinForm.nn_mem.focus();
		return;
	}
	
	var str=document.signinForm.email.value;
	var atPos=str.indexOf('@');
	var atLastPos=str.lastIndexOf('@');
	var dotPos=str.indexOf('.');
	var spacePos=str.indexOf(' ');
	var commaPos=str.indexOf(',');
	var eMailSize=str.lengh;
