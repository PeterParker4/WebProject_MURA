...
function idCheck(id_mem) {
	if (id_mem == "") {
		...
		url = "/MURA2/page/member/idCheck.mur?cmd=&id_mem=" + id_mem
...
function inputCheck2() {
	if (document.modifyForm.nn_mem.value == "") {
		alert("닉네임을 입력해주세요.");
		document.modifyForm.nn_mem.focus();
		return;
	}
	if (document.modifyForm.email_mem.value == "") {
		alert("이메일을 입력해주세요.");
		document.modifyForm.email_mem.focus();
		return;
	}

	var str = document.modifyForm.email_mem.value;
	var atPos = str.indexOf('@');
	var atLastPos = str.lastIndexOf('@');
	var dotPos = str.indexOf('.');
	var spacePos = str.indexOf(' ');
	...
	if (document.modifyForm.tel_mem.value == "") {
		alert("전화번호를 입력해주세요.");
		document.modifyForm.tel_mem.focus();
		return;
	}
	
	if (document.modifyForm.zipcode_mem.value == "") {
		alert("우편번호를 입력해 주세요.");
		document.modifyForm.zipcode_mem.focus();
		return;
	}

	if (document.modifyForm.zc1_mem.value == "") {
		alert("주소를 입력해 주세요.");
		document.modifyForm.zc1_mem.focus();
		return;
	}

	if (document.modifyForm.zc2_mem.value == "") {
		alert("상세 주소를 입력해 주세요.");
		document.modifyForm.zc2_mem.focus();
		return;
	}
	
	if (document.modifyForm.pw_mem.value == "") {
		alert("비밀번호를 입력해주세요.");
		document.modifyForm.pw_mem.focus();
		return;
	}
	if (document.modifyForm.repass_mem.value == "") {
		alert("비밀번호를 확인해주세요.");
		document.modifyForm.repass_mem.focus();
		return;
	}
	if (document.modifyForm.pw_mem.value != document.modifyForm.repass_mem.value) {
		alert("비밀번호가 일치하지 않습니다.");
		document.modifyForm.repass_mem.focus();
		return;
	}
...
function sendAddress2(zipcode, sido, gugun, dong, ri, bunji) {
	var address = sido + " " + gugun + " " + dong + " " + ri + " " + bunji;
	opener.document.modifyForm.zipcode_mem.value = zipcode;
	opener.document.modifyForm.zc1_mem.value = address;
}
